...
# base_path = '/gpfs/home2/jbao/NS_Thesis/data/filtered/full_dataset_partitioned/regression'
...
# df_val = duckdb.query(f"SELECT * FROM '{base_path}/dataset_group=validation/data_0.parquet'").to_df()
df_test = duckdb.query(f"SELECT * FROM '{base_path}/dataset_group=test/data_0.parquet'").to_df()
...
query = f"""
SELECT * FROM '{base_path}/dataset_group=train/data_0.parquet'
UNION ALL
SELECT * FROM '{base_path}/dataset_group=validation/data_0.parquet'
ORDER BY hour_bin  -- Optional: Sort by time immediately
"""

df_train = duckdb.query(query).to_df()

# Remove unnecessary columns that won't be used as features, TODO: move to EDA/preprocessing instead if needed
# e.g. dropping manual cyclic features since we will use built-in cyclic encoders in Darts, TODO: should preprocess cyclic features if we want to keep them later
cols_to_drop = ['hour_of_day', 'day_of_week', 'month_of_year','holiday_name', 'day_of_week_1', 'month_of_year_1', 'hour_of_day_1']
...
covariate_cols = [c for c in df_train.columns if c not in ['hour_bin', 'avg_delay_minutes', 'station_code']]
future_covs_cols = ['is_holiday', 'is_weekend', 'is_rush_hour']
...
"""## Make Predictions"""
naive_forecast = [naive_model.predict(28, series=ts) for ts in series_test_scaled]
xgb_forecast = [xgb_model.predict(28, series=ts,
                                 past_covariates=past_cov_test_scaled,
                                 future_covariates=future_cov_test_scaled) for ts in series_test_scaled]
lgbm_forecast = [lgbm_model.predict(28, series=ts,
                                   past_covariates=past_cov_test_scaled,
                                   future_covariates=future_cov_test_scaled) for ts in series_test_scaled]
print("Inverse transforming forecasts...", flush=True)
# Inverse transform the forecasts to original scale
naive_forecast = [target_scaler.inverse_transform(forecast) for forecast in naive_forecast]
xgb_forecast = [target_scaler.inverse_transform(forecast) for forecast in xgb_forecast]
lgbm_forecast = [target_scaler.inverse_transform(forecast) for forecast in lgbm_forecast]
original_test_series = [target_scaler.inverse_transform(ts) for ts in series_test_scaled]

print("Creating result files...", flush=True)
"""## Create result Files"""
...
for i, ts in enumerate(series_test):
    ...
    naive_results = pd.concat([
        naive_results,
        pd.DataFrame({'station_code': station_code, **{f"F{i+1}": val for i, val in enumerate(naive_values)}})
    ])
    xgb_results = pd.concat([
        xgb_results,
        pd.DataFrame({'station_code': station_code, **{f"F{i+1}": val for i, val in enumerate(xgb_values)}})
    ])
    lgbm_results = pd.concat([
        lgbm_results,
...
metrics_data = {
    ...
    "Naive Model": [
        np.mean(rmse(original_test_series, naive_forecast)),
        np.mean(mae(original_test_series, naive_forecast)),
        np.mean(r2_score(original_test_series, naive_forecast))
    ],
    "XGB Model": [
        np.mean(rmse(original_test_series, xgb_forecast)),
        np.mean(mae(original_test_series, xgb_forecast)),
        np.mean(r2_score(original_test_series, xgb_forecast))
    ],
    "LGBM Model": [
        np.mean(rmse(original_test_series, lgbm_forecast)),
        np.mean(mae(original_test_series, lgbm_forecast)),
        np.mean(r2_score(original_test_series, lgbm_forecast))
    ]
} 
